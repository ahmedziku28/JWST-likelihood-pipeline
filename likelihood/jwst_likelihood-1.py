    ...
    def logp(self, **params_values):
        ...
        try:
            ...
            for i, (z_min, z_max, z_mid) in enumerate(self._zbins):
                ...
                # Comoving survey volume [Mpc^3] using current CLASS cosmology
                V = V_survey(cosmo, z_min, z_max)
                if not np.isfinite(V) or V <= 0.0:
                    return -np.inf

                M_star_thr, rho_obs, rho_low, rho_high = compute_observed_rho_star(self._catalog, z_min, z_max, V)

                # Theory rho_star from HMF + SHMR
                M_h, dndlnm, _sigma = compute_hmf(cosmo, z_mid)
                ...
                # Split-Gaussian sigma
                # theory > obs -> penalise with upper error bar (rho_high - rho_obs)
                # theory <= obs -> penalise with lower error bar (rho_obs - rho_low)
                sigma = np.where(
                    rho_theory > rho_obs,
                    rho_high - rho_obs,
                    rho_obs  - rho_low,
                ...
                # Guard degenerate sigma values (should not occur with clean data)
                ...
                # Accumulate split-Gaussian chi-squared
